...
class ConsoleWriter:
    ...
    def get_nics(self):
            if self.nics:
                    return self.nics
            self.nics = ethtool.get_active_devices()
            return self.nics
    ...
    def save(self,cpu_list, thread_list, filename):
            ...
            for name in kthreads.keys():
                    ...
                    if (cpu_list and not set(kt.affinity).intersection(set(cpu_list))) or \
                       (thread_list and kt.pid not in thread_list) :
                            del kthreads[name]
            tuna.generate_rtgroups(filename, kthreads, self.get_nr_cpus())

    def ps_show_header(self,has_ctxt_switch_info,cgroups = False):
            print "%7s %6s %5s %7s       %s" % \
                    (" ", " ", " ", _("thread"),
    ...
    def ps_show_sockets(self,pid, ps, inodes, inode_re, indent = 0):
    ...
    def format_affinity(self,affinity):
            ...
            return ",".join(str(hex(a)) for a in procfs.hexbitmask(affinity, self.get_nr_cpus()))

    def ps_show_thread(self,pid, affect_children, ps,
    ...
    def ps_show(self,ps, affect_children, thread_list, cpu_list,
            ...
            for pid in ps_list:
                    self.ps_show_thread(pid, affect_children, ps,
                                   has_ctxt_switch_info, sock_inodes,
                                   sock_inode_re, cgroups)

    def load_socktype(self,socktype, inodes):
    ...
    def load_sockets(self):
            ...
            for socktype in (inet_diag.TCPDIAG_GETSOCK,
                    ...
                    self.load_socktype(socktype, inodes)
            return inodes

    def do_ps(self,thread_list, cpu_list, irq_list, show_uthreads,
            ...
            sock_inode_re = None
            if show_sockets:
                    sock_inodes = self.load_sockets()
                    sock_inode_re = re.compile(r"socket:\[(\d+)\]")

            has_ctxt_switch_info = self.ps[1]["status"].has_key("voluntary_ctxt_switches")
            try:
                    self.ps_show_header(has_ctxt_switch_info, cgroups)
                    self.ps_show(self.ps, affect_children, thread_list,
    ...
    def find_drivers_by_users(self,users):
    ...
    def show_irqs(self,irq_list, cpu_list):
            if not self.irqs:
                    self.irqs = procfs.interrupts()

            print "%4s %-16s %8s" % ("#", _("users"), _("affinity"),)
            sorted_irqs = []
            for k in self.irqs.keys():
                    try:
                            irqn = int(k)
                            affinity = self.irqs[irqn]["affinity"]
                    except:
                            continue
            ...
            for irq in sorted_irqs:
                    affinity = self.format_affinity(irqs[irq]["affinity"])
                    users = irqs[irq]["users"]
                    print "%4d %-16s %8s" % (irq, ",".join(users), affinity),
                    drivers = self.find_drivers_by_users(users)
                    if drivers:
                            print " %s" % ",".join(drivers)
                    else:
                            print

    def do_list_op(self,op, current_list, op_list):
            if not current_list:
                    current_list = []
            if op == '+':
    ...
    def irq_mapper(self,s):
            ...
            irq_list_str = self.irqs.find_by_user_regex(re.compile(fnmatch.translate(s)))
            irq_list = []
    ...
    def apply_config(self,filename):
...
def main():
	...
	for o, a in opts:
		if o in ("-h", "--help"):
			...
			cpu_list = consolewriter.do_list_op(op, cpu_list, op_list)
			...
			op_list = reduce(lambda i, j: i + j,
					 map(consolewriter.thread_mapper, a.split(",")))
			op_list = list(set(op_list))
			thread_list = consolewriter.do_list_op(op, thread_list, op_list)
			...
			if not op_list and type(a) == type(''):
				thread_list_str = consolewriter.do_list_op(op, thread_list_str,
							     a.split(","))
			if not op:
				irq_list = None
		elif o in ("-f", "--filter"):
			...
			if not thread_list and not irq_list:
				if thread_list_str or irq_list_str:
					continue
			consolewriter.do_ps(thread_list, cpu_list, irq_list, uthreads,
			...
			if not irq_list and irq_list_str:
				continue
			consolewriter.show_irqs(irq_list, cpu_list)
			...
			consolewriter.save(cpu_list, thread_list, a)
		elif o in ("-S", "--sockets"):
			(op, a) = pick_op(a)
			...
			(op, a) = pick_op(a)
			op_list = reduce(lambda i, j: i + j,
					 map(consolewriter.irq_mapper, list(set(a.split(",")))))
			irq_list = consolewriter.do_list_op(op, irq_list, op_list)
			# See comment above about thread_list_str
			if not op_list and type(a) == type(''):
				irq_list_str = consolewriter.do_list_op(op, irq_list_str,
							  a.split(","))
			if not op:
				thread_list = []
			if not ps:
